...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pdf_path = PathBuf::from(args.next().ok_or("usage: extract_fixture <pdf> [password]")?);
    let password = args.next();

    let bytes = std::fs::read(&pdf_path)?;
    let pages = trackery_core::pdf::open_statement(&bytes, password.as_deref())?;

    let stem = pdf_path.file_stem().and_then(|s| s.to_str()).unwrap_or("statement");
    for (i, text) in pages.iter().enumerate() {
        let out = pdf_path.with_file_name(format!("{stem}_page{}.txt", i + 1));
        std::fs::write(&out, text)?;
        eprintln!("wrote {}", out.display());
    }
    eprintln!("{} page(s). Redact per tests/fixtures/README.md before committing.", pages.len());
    Ok(())
}
